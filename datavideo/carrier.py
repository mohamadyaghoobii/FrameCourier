...
from .segmenter import encode_distributed_segments, temporary_segment_dir
...
def build_distributed_mux_command(cover_video, encoded_segments, output_path):
    ...
    for i in range(len(encoded_segments)):
        ...
        cmd.extend(["-metadata:s:v:" + str(video_stream_number), "title="])
        cmd.extend(["-metadata:s:v:" + str(video_stream_number), "language=und"])
...
def embed_file_distributed(input_path, output_path, cover_video=None, default_dir="default", segments=1, schedule="even", seed=1337, width=1920, height=1080, fps=30, password=None, progress=True, keep_segments=False, temp_dir=None):
    ...
    if progress:
        ...
        print("data segments requested:", segments)
        print("schedule:", schedule)
        print("encryption:", "on" if password is not None else "off")
